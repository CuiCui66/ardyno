...
class DynamixelMotor:public DynamixelDevice
{
	...
	DynamixelStatus getCurrentSpeed(uint16_t &spd);
	DynamixelStatus getCurrentLoad(uint16_t &ld);
	DynamixelStatus getCurrentVoltage(uint8_t &volt);
	DynamixelStatus getCurrentTemperature(uint8_t &temp);
